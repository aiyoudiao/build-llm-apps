...
### [Day 11: 多模态大模型 (Multimodal LLM)](./day11/README.md)
深入探索视觉-语言模型在保险、视频分析和文档处理三大场景的实战应用。

> 🔗 **传送门**：[点击查看 Day 11 详细文档与代码](./day11/README.md)

- **VL 模型在保险行业的应用**：
  - **批量图片分析**：基于 Qwen-VL-Max 的车险定损批量处理系统，从 Excel 读取理赔指令，自动回写分析结果。
  - **本地图片识别**：使用 DashScope 原生 SDK 调用 Qwen-VL-Plus 进行基础多模态对话。
  - **多轮对话与定位**：演示视觉定位（返回物体坐标）和本地 GPU 环境下的自动边界框绘制。
- **VL 模型对视频的应用**：
  - **InternVL 2.5 本地方案**：实现智能模型生命周期管理、动态视觉分块、自适应时空采样、BFloat16 低显存推理优化。
  - **Qwen-VL 云端方案**：基于 DashScope API 的极简部署，直接上传视频进行多轮对话。
  - **多模型轮询**：当模型额度耗尽时自动切换到下一个可用模型，通过上下文重建保持对话连贯性。
- **MinerU 在文档中的应用**：
  - **本地命令行**：使用 MinerU 工具本地处理 PDF 文档，支持实时日志输出和错误过滤。
  - **云端 API**：通过 MinerU API 进行文档提取，支持 OCR、公式提取等高级功能。

### [Day 12: 大模型底层数学原理与推荐系统](./day12/README.md)
深入探索大模型微调与推荐系统背后的核心数学原理，包括矩阵分解与低秩近似的纯代码实现。

> 🔗 **传送门**：[点击查看 Day 12 详细文档与代码](./day12/README.md)

- **ALS 矩阵分解推荐系统**：
  - **手写线性代数库**：纯 Python 实现矩阵转置、乘法、高斯-约旦消元法求逆等底层运算。
  - **协同过滤**：通过交替最小二乘法（ALS）分解用户-物品评分矩阵，实现 Top-N 个性化电影推荐。
- **SVD 图像压缩与大模型微调模拟**：
  - **奇异值分解**：利用 SVD 对灰度及 RGB 多通道图像进行分解提取核心特征。
  - **低秩近似**：通过保留前 k 个最大奇异值重构图像，直观演示数据降维与特征保留，深刻理解 LoRA 等参数高效微调（PEFT）算法的数学本质。
